...
public class ProductService implements BaseService<Product> {
    ...
    public String getAll() {
        ...
        if (optional.isPresent()) {
            ...
            list.forEach((product) -> {
                String productInfo = String.format(Locale.US, "%d) id - %d, %s %d pcs, %.2f USD", count.incrementAndGet(),
                        product.getId(), product.getName(), product.getQuota(), product.getPrice());
    ...
    public String getById(String id) {
        ...
        if (optional.isEmpty()) {
            ...
            return String.format(Locale.US, "id - %d, %s %d pcs, %.2f USD",
                    product.getId(), product.getName(), product.getQuota(), product.getPrice());
        }
    }
